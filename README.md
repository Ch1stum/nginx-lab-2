Лабораторная работа №2: Nginx + PHP-FPM (Вариант 15)
👩‍💻 Автор
ФИО: Косинский Максим Сергеевич

Группа: ПМ2-ИП1

📌 Описание задания
Научиться конфигурировать веб-сервер Nginx для работы с PHP через PHP-FPM.
Освоить базовые принципы PHP (на примере phpinfo()).
Повторить основы HTML: работа с формами, различными типами полей ввода.
Освоить базовую обработку форм с помощью JavaScript без перезагрузки страницы.
Результат доступен по адресу http://localhost:8080.

⚙️ Как запустить проект
Клонировать репозиторий:

git clone https://github.com/Ch1stum/nginx-lab-2.git
cd nginx-lab-2
Запустить контейнеры:

docker-compose up -d --build
Открыть в браузере: http://localhost:8081 📂 Содержимое проекта

docker-compose.yml — описание сервиса Nginx

www/form.html — главная HTML-страница

nginx/default.conf — файл для обработки PHP

screenshots/ — скриншоты

✅ Результат Сервер в Docker успешно запущен, Nginx отдаёт мою HTML-страницу с тестовой формой мастер-класса
